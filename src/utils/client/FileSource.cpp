...
char* ToISO8601(FILETIME ft) {
    ...
    char* buf =
        str::Format("%04d-%02d-%02dT%02d:%02d:%02dZ", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
    return buf;
}
...
const char* GuessMimeType(const char* fileName) {
    ...
    for (const auto& e : table) {
        ...
        if (str::EqIS(ext, e.ext)) return e.mime;
    }
