...
class UploadSession {
  public:
    ...
    SendReport Send(const char* host, int port, int workerCount, int chunkSize,
    ...
    SendReport Send(const char* host, int port, int workerCount, int chunkSize,
                    ...
                    // const char* pin = "",
                    void*) {
        return Send(host, port, workerCount, chunkSize, files, Func1<UploadProgress*>());
    };

  private:
    void Cancel(const char* host, int port, const char* sessionId);
