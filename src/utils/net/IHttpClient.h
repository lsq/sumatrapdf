...
class IHttpClient {
  public:
    ...
    virtual HttpResponse Post(const char* host, int port, const char* path, StrBuilder* contentType,
                              StrBuilder* body) = 0;
#if 1
    // POST che invia in streaming il contenuto di un file come body.
    // 'progress' (opzionale) viene chiamato dopo ogni chunk inviato.
    virtual HttpResponse PostFile(const char* host, int port, const char* path, StrBuilder* contentType,
